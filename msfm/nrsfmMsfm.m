function anim = nrsfmMsfm( animGT, d )
...
anim = oneGradientDescentIter(animRSC, zeros(3*nPoint,nFrame), grt, grR, grS, ...
lamt, lamR, lamS);
...
while (errPrev-err)>0.001*errPrev
  ...
  grSTangent = grS - bsxfun( @times, sum( bsxfun(@times, ...
    reshape(grS,3*nPoint,1,nFrame), H), 1 ), H );
